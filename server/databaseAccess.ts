...
export async function getRoundsData(matchId: number) {
  try {
    const sql = "select r.winner_id, count(round_id) as roundswon from round as r join `match` as m on r.match_id = m.match_id where r.match_id = :matchId group by winner_id;"
