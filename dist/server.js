...
import { createInitialHand, startGame, checkForExistingGame, getRoundState, logMove, getCurrentHand, test, validateUser, countTotalMoves, getLatestOppMove, createPlayer, getLobbyData, getUsernameById, } from "../server/databaseAccess";
...
async function createServer() {
    ...
    app.post("/api/register", async (req, res) => {
        ...
        try {
            const result = await createPlayer({ username, email, password });
            if (result) {
    ...
    app.post("/api/playerhand", async (req, res) => {
        ...
        if (hand.hand?.length === 0) {
            const newHand = await createInitialHand(params.choice, params.player, params.round);
    ...
    app.post("/api/currentgame", async (req, res) => {
        if (!req.session?.playerId) {
            res.json({
                success: false,
                data: "Session Error - could not authenticate player",
    ...
    app.post("/api/logmove", async (req, res) => {
        if (!req.session?.playerId) {
            res.json({
                success: false,
                data: "Session Error - could not authenticate player",
